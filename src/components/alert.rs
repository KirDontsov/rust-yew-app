...
pub fn alert_component(props: &Props) -> Html {
	...
	html! {
	<div id="myToast" class={format!("fixed top-14 right-10 px-5 py-2 border-r-8 border-indigo-300 bg-gray-700 drop-shadow-lg rounded-md {}", if show_alert { "" } else { "hidden" })}>
		<p class="text-sm text-white">
			<span class="mr-2 inline-block px-3 py-1 rounded-full bg-indigo-400 text-white font-extrabold">{"i"}</span>
			{props.message.clone()}
		</p>
	</div>
	}

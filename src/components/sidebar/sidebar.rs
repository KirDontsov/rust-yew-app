	   ...
	   <div class="h-full px-3 py-4 overflow-y-auto bg-gray-50 dark:bg-gray-800">
		  <ul class="space-y-2 font-medium">
			 <SidebarItem to={Route::DashboardPage} active={location.path() == "/dashboard"}>
			   <svg class={format!("w-5 h-5 transition duration-75 text-gray-400  {}", if location.path() == "/dashboard" {"text-indigo-400 group-hover:text-indigo-300"} else {"text-gray-400 group-hover:text-white"})} aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 22 21">
			...
			<SidebarItem to={Route::ParserPage} active={location.path() == "/parser"}>
			   ...
			   <span class="flex-1 ml-3 whitespace-nowrap">{"Parser"}</span>
			</SidebarItem>
				<li>

...
pub fn form_input_component(props: &Props) -> Html {
	...
	html! {
	<div>
	  <label html={props.name.clone()} class="block text-indigo-400 mb-3">
		{props.label.clone()}
	  </label>
	  <input
		type={input_type}
		placeholder=""
		class="block w-full appearance-none bg-gray-700 text-white focus:outline-none py-2 px-4 rounded-md"

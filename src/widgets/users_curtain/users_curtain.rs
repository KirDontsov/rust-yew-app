use crate::{api::User, components::Curtain};
use yew::prelude::*;

#[derive(PartialEq, Properties)]
pub struct Props {
	pub selected_user: UseStateHandle<Option<User>>,
...
pub fn users_curtain_component(props: &Props) -> Html {
	...
	let selected_user = props.selected_user.as_ref().clone();
	...
	html! {
	  ...
	  { if selected_user.is_some() { selected_user.unwrap().name.clone() } else { "Title".to_string() }}</h5>
